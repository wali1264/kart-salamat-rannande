...
export const Verify: React.FC<{ cardId: string }> = ({ cardId }) => {
  ...
  const isTeacher = driver.type === 'teacher';
      ...
      >
        ...
        <div className="flex items-center justify-center mb-8">
           <div className="bg-white px-6 py-3 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-3">
              <div className={`w-10 h-10 ${isTeacher ? 'bg-emerald-600' : 'bg-blue-700'} rounded-xl flex items-center justify-center`}>
                 <ShieldCheck className="w-6 h-6 text-white" />
              </div>
              <div className="text-right">
                 <h1 className="text-xs font-bold text-slate-800">{isTeacher ? 'سامانه مدیریت هویت اساتید معارف' : 'سامانه مدیریت مکتب و هویت شاگردان'}</h1>
        ...
        <div className="bg-white rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-100">
          {/* Validity Banner */}
          <div className={`p-4 text-center text-white font-bold text-sm tracking-wide ${isExpired ? 'bg-rose-500' : isTeacher ? 'bg-emerald-500' : 'bg-blue-600'}`}>
             {isExpired 
               ? (isTeacher ? 'کارت استاد منقضی شده است (Expired)' : 'کارت شاگرد منقضی شده است (Expired)') 
               : (isTeacher ? 'کارت استاد معتبر و اصلی است (Verified)' : 'کارت معتبر و اصلی است (Verified)')}
          </div>

          <div className="p-8">
             <div className="flex flex-col md:flex-row items-center gap-8 mb-8">
                <div className={`w-32 h-40 bg-slate-50 rounded-3xl border border-slate-100 overflow-hidden shadow-inner flex items-center justify-center bg-cover bg-center ${isTeacher ? 'ring-2 ring-emerald-50' : 'ring-2 ring-blue-50'}`} 
                ...
                <div className="flex-1 space-y-4 w-full">
                   <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="space-y-1">
                         <p className="text-[10px] text-slate-400 font-bold uppercase">{isTeacher ? 'کد شناسایی' : 'نمبر اساس'}</p>
                         <p className="font-bold text-slate-800 font-mono tracking-wider">{driver.student_id_no || driver.license_number}</p>
                      </div>
                      <div className="space-y-1 text-left md:text-right">
                         <p className="text-[10px] text-slate-400 font-bold uppercase">{isTeacher ? 'بست / رتبه' : 'صنف'}</p>
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 text-center">
                   <Activity className={`w-5 h-5 ${isTeacher ? 'text-emerald-600' : 'text-blue-600'} mx-auto mb-2`} />
                ...
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 text-center">
                   <Calendar className={`w-5 h-5 ${isTeacher ? 'text-emerald-600' : 'text-indigo-600'} mx-auto mb-2`} />
                ...
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 text-center">
                   ...
                   <p className="text-[10px] text-slate-400 font-bold mb-1 uppercase">وضعیت فعالیت</p>
                   <p className="text-sm font-bold text-emerald-600">فعال (Active)</p>
                </div>
             </div>

             <div className={`flex items-start gap-3 p-4 ${isTeacher ? 'bg-emerald-50 border-emerald-100' : 'bg-blue-50 border-blue-100'} rounded-2xl border`}>
                <ShieldCheck className={`w-5 h-5 ${isTeacher ? 'text-emerald-600' : 'text-blue-600'} flex-shrink-0 mt-0.5`} />
                <p className={`text-[10px] ${isTeacher ? 'text-emerald-800/80' : 'text-blue-800/80'} leading-relaxed`}>
                   {isTeacher 
                     ? 'این اطلاعات مستقیماً از سامانه مرکزی وزارت معارف استخراج شده و نشان‌دهنده اصالت کارت هویت استاد می‌باشد. هرگونه جعل پیگرد قانونی دارد.'
