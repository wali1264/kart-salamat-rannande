...
export const cleanPhoneSuffix = (val: string): string => {
  let clean = val.replace(/\D/g, '');
  if (clean.startsWith('0093')) {
    clean = clean.slice(4);
  } else if (clean.startsWith('93')) {
    clean = clean.slice(2);
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (phoneDigits.length !== 9 || !phoneDigits.startsWith('7')) {
        setError('لطفاً شماره تماس معتبر ۹ رقمی معتبر افغانستان بدون صفر اول وارد کنید (مثال: 799123456).');
        setLoading(false);
        return;
      }
